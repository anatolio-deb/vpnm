...
LINKS = {
    ...
    "https://github.com/cloudflare/cloudflared/releases/download/2021.5.10/cloudflared-linux-amd64": "cloudflared-linux-amd64",
...
for link, filename in LINKS.items():
    if filename in ["install-release.sh", "tun2socks-linux-amd64.zip"]:
...
subprocess.run(["systemctl", "restart", "vpnmd.service"], check=True)
